      ...
      {data && data.contacts?.map((el) => (
        <li className={styles.contacts__listItem} key={el.uuid}>
          ...
          {el.type && mailRegex.test(el.type)
            ? <a href={el.value ? el.value : ''} target="_blank" rel="noreferrer" className={styles.contacts__link}>{el.value}</a>
            : <a href={el.value ? el.value : ''} target="_blank" rel="noreferrer" className={styles.contacts__phoneLink}>{el.value}</a>}
        </li>
      ))}
    </ul>
  </Container>
