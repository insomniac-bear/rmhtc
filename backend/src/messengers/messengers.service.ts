...
import { createMessengerTypeDto, MessengerTypeDto } from './dto/messenger-type.dto';
...
export class MessengersService {
  constructor(
    @Inject(MESSENGER_REPOSITORY) private readonly messengerEntity: typeof Messenger,
    @Inject(MESSENGER_TYPE_REPOSITORY) private readonly messengerTypeEntity: typeof MessengerType,
  ) {}

  async getAllTypesOfMessenger(): Promise<MessengerTypeDto[]> {
    const rawTypesData =  await this.messengerTypeEntity.findAll();
    return rawTypesData.map(messengerType => createMessengerTypeDto(messengerType));
  }
